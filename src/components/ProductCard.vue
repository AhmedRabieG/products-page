  ...
  product: {
    ...
    validator: (value) => {
      ...
      if (typeof value.id !== "string" && typeof value.id !== "number") {
        console.error(
          `Invalid product id type  ${typeof value.id} and ID is ${value.id}`
        );
        isValid = false;
      }
...
<template>
  <a
    href="#"
    class="group bg-white border border-gray-200 shadow-lg rounded-lg h-full hover:shadow-xl flex flex-col hover:border-blue-500 transition-all duration-300"
  >
    <div
      v-if="product.image"
      class="w-full aspect-[1.2] overflow-hidden rounded-lg"
    >
      <img
        class="w-full h-full object-cover rounded-lg transition-transform duration-300 group-hover:scale-[1.2]"
        :src="product.image"
        :alt="product.name"
      />
    </div>
    <div
      v-else
      class="relative flex items-center justify-center bg-[#eee] w-full aspect-[1.2] overflow-hidden rounded-lg"
    >
    ...
    <div class="p-3 text-start flex flex-col flex-1">
      ...
      <p class="text-xl font-bold text-gray-800 mb-2">${{ product.price }}</p>
